...
Game::Game(sf::RenderWindow &window,sf::View &camera,SoundManager &soundManager)
{
    ...
    this->addButton(State::Idle,Button(ButtonType::Hover, sf::FloatRect(sf::Vector2f(0,windowSize.y-100.f),sf::Vector2f(windowSize.x,100)) ,[this,&soundManager](){currentState=State::CameraState;soundManager.playNoise("cameraOpen");
    std::cout<<"je switch en state camera"<<std::endl;},true));
    this->addButton(State::CameraState,Button(ButtonType::Switch, sf::FloatRect(sf::Vector2f{0,0},sf::Vector2f{100,100}) ,[this,&soundManager](){currentState=State::Idle;soundManager.playNoise("cameraOpen");
